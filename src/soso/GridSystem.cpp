...
float GridSystem::colX(int iNumber)
{
    if(iNumber < 0)
        return 0;
    else
...
float GridSystem::col(int iNumber)
{
    return (colW * iNumber + leading * (iNumber - 1));
}

void GridSystem::buildGrid(float iPageWidth, float iPageHeight, int iNumColumns, float iBaselineLeading, float iLeft, float iRight, float iHead, float iTail)
{
    width = iPageWidth;
    height = iPageHeight;
...
void GridSystem::buildLines()
{
    ...
    for(int i=0; i < (numCol+1); i++){
        ...
        if(i>0){
            auto line = make_shared<LineStripObject>(2);
    ...
    //Head line
    auto headLine = make_shared<LineStripObject>(2);
    ...
    //Tail line
    auto tailLine = make_shared<LineStripObject>(2);
...
void GridSystem::setLineWidth(float iWidth)
{
    for(int i=0; i < lines.size(); i++)
...
bool GridSystem::isGridVisible()
{
    return isGridShown;
}

bool GridSystem::isBaselineGridVisible()
{
    return isBaselineGridShown;
}

//By default this shows all lines in grid. To hide baseline grid, use showBaselines()
void GridSystem::showLines(bool iShowHide)
{
    for(int i=0; i < lines.size(); i++){
...
void GridSystem::showBaselines(bool iShowHide)
{
    for(int i=0; i < baselineLines.size(); i++){
