...
ofxSosoMappedChar::ofxSosoMappedChar(unsigned char iMapToIndex, int iUnicodeIndex, string iNamedEntity, char iUTFByte0, char iUTFByte1, char iUTFByte2, char iUTFByte3, char iUTFByte4, char iUTFByte5)
{
  ...
  //TODO: not working with the unicode chars from 0->255
  
  //NOTE: this assumes that the sequence doesn't have a zero break within in
  if (iUTFByte0 != 0) utf8Sequence += iUTFByte0;
  else {
    //sequences for unicode 0 to 255 start with a 0x00
    if (iUTFByte1 != 0) utf8Sequence += iUTFByte0;
  }
...
void ofxSosoTrueTypeFont::buildMappedChars()
{
	if(!areMappedCharsBuilt){
		...
		//namedEntityChars.push_back(new ofxSosoMappedChar(0xA9, 0xC2A9, "&copy;", 0xC2, 0xA9)); //TODO: named entities are NOT working
    mappedChars.push_back(new ofxSosoMappedChar(0xA9, 0x00A9, "&copy;", 0xC2, 0xA9)); //TODO: add the sequence here
...
void ofxSosoTrueTypeFont::replaceNamedEntities(string &iString)
{
	for(int i=0; i < namedEntityChars.size(); i++){
		...
		while(pos != string::npos){	//search for multiple copies of named entity in string
			iString.replace(pos, namedEntityChars[i]->namedEntity.length(), 1, namedEntityChars[i]->mapToIndex);
